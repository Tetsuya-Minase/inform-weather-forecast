...
export class ConverterServiceImpl implements ConverterServer{
    ...
    public domDataFormatter(domList) {
        ...
        for (const dom of domList) {
            const data = dom.textContent.replace(/ /g, '').split('\n').filter(r => r !== '');
            returnList.push(data);
        }
        return returnList;
    }

    public list2FullInformation(indexList: Array<Array<string>>, pictDataList: Array<Array<string>>, temperatureDataList: Array<Array<string>>) {
    ...
    public list2TodayDetailInformation(list: Array<Array<string>>, pictDataList: Array<Array<string>>, temperatureDataList: Array<Array<string>>) {
        const indexData = {};
        // 天気設定
